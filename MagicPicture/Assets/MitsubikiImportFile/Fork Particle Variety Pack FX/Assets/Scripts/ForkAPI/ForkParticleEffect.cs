...
{	
	...
	const string XMLExtensionPrefix = "PSK";
	...
	#if UNITY_IOS
	[MonoPInvokeCallback(typeof(frkSetTextureNameCB))]
	#endif
    ...
    private void ForkEffectCreate() {
		...
		TextAsset effectFile = Resources.Load(sEffectPathName) as TextAsset;
		...
		if (effectFile != null) {
			...
			byte[] sName = System.Text.Encoding.UTF8.GetBytes(sEffectName);

			pFrkEffect = _frkParticlePluginCreateEffect(effectFile.bytes, sName, effectFile.bytes.Length);
            if (pFrkEffect == IntPtr.Zero) {
